function [steps, anss]  = Runge_kutta_4order_2Order(f, g ,x0, y0, z0, h, n)
    ...
    syms x y z;
    f = @(xTmp, yTmp, zTmp) GRounder(subs(f, [x y z], [xTmp yTmp zTmp]));
    g = @(xTmp, yTmp, zTmp) GRounder(subs(g, [x y z], [xTmp yTmp zTmp]));
    ...
    for i=1:n
        ...
        xTmp = xTmp+h;
        yTmp = GRounder(yTmp + h*(k1fTmp+2*k2fTmp+2*k3fTmp+k4fTmp)/6);
        zTmp = GRounder(zTmp + h*(k1gTmp+2*k2gTmp+2*k3gTmp+k4gTmp)/6);
    ...
    for i=0:10:10*n-10
        steps{i+1} = ['k1f(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+1}))];
        steps{i+2} = ['k1g(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+2}))];
        steps{i+3} = ['k2f(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+3}))];
        steps{i+4} = ['k2g(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+4}))];
        steps{i+5} = ['k3f(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+5}))];
        steps{i+6} = ['k3g(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+6}))];
        steps{i+7} = ['k4f(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+7}))];
        steps{i+8} = ['k4g(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+8}))];
        steps{i+9} = ['y(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+9}))];
        steps{i+10} = ['z(', char(GRounder(x0+h*(i/10+1))), ') = ', char(GRounder(out{i+10}))];
